...
int main(void)
{
	...
	sem_init(&sem, 0, 0); // 创建用于线程之间(pshared = 0)同步的信号量
	// 因为同一进程内的线程之间共享主进程的资源，所以不需要将信号量放到共享内存区域中
	...
	pthread_join(pthread1, NULL); // 等待子线程1结束，回收其资源
	pthread_join(pthread2, NULL); // 等待子线程2结束，回收其资源
