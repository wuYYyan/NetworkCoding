...
int main(void)
{
	...
	// TCP使用流式套接字: SOCK_STREAM；UDP使用数据报套接字: SOCK_DGRAM
	...
	// 在参数传递时，把sockaddr_in结构体强制转换为sockaddr结构体，因为bind()函数的第二个参数是sockaddr结构体类型的指针
	...
	while(1) // 服务器一直运行等待
	{
		...
		// accept()函数返回一个新的套接字描述符，用于与客户端通信
		printf("Client IP address = %s\n", inet_ntoa(client_addr.sin_addr)); 
		// inet_ntoa()函数将网络字节序二进制的IP地址转换为点分十进制的IP地址
